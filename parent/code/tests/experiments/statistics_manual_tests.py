...
if __name__ == "__main__":
    connecties_nationaal = pd.read_csv("parent/data/ConnectiesHolland.csv", header = None, names = ["station1", "station2", "time"], index_col = False)

    sum_time = connecties_nationaal["time"].sum()
    print(f"Total time for nationaal: {sum_time}")

    min_trains = sum_time / 120
    print(f"Minimum number of trains: {min_trains}")
