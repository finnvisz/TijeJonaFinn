...
if __name__ == "__main__":
    ...
    # plot_autorun_hillclimber(project_name="agile_zaterdag", title="Hillclimber: 24 uur zaterdag", use_aggregated=True)

    end_scores = read_scores_from_csv("parent/code/algorithms/autorun_hillclimber/agile_zaterdag/end_scores.csv", custom_file_path=True)
    plot_scores_fancy(end_scores, title="Hillclimber 24 uur zaterdag: verdeling van eindscores", binwidth=10, xlim=(min(end_scores), max(end_scores)), save_to_pdf=True)
